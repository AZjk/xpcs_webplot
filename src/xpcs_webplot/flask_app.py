...
def get_subdirectories(html_folder):
    ...
    for item in os.listdir(html_folder):
        ...
        if has_results:
            ...
            subdirs.append({"name": item, "path": item, "result_count": result_count})
...
def create_app(html_folder=DEFAULT_HTML_FOLDER):
    ...
    def api_data():
        ...
        if filename_filter or analysis_filter:
            ...
            for item in results:
                ...
                if (
                    analysis_filter
                    and analysis_filter not in item["analysis_type"].lower()
                ):
                    continue
    ...
    def view_subdir(subdir_name):
        ...
        return render_template(
            "flask_subdir.html", subdir_name=subdir_name, results=results
        )
...
def main():
    ...
    from .cli import _BROWSER_BLOCKED_PORTS

    if args.port in _BROWSER_BLOCKED_PORTS:
        print(
            f"Error: Port {args.port} is blocked by browsers. "
